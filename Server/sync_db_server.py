...
class LocalSyncManager:
    ...
    def openDatabase(self):
        ...
        self.LocalSyncModel._meta.database = db
        self.LocalSyncDataModel._meta.database = db
        return db
    ...
    def ready(self):
        ...
        db.create_tables([self.LocalSyncModel, self.LocalSyncDataModel])
        dbMgr = DbTableManager()
        for m in dbMgr.getAllModels():
            ut = dbMgr.getMaxUpdateTime(m['ormClass'])
            self.LocalSyncModel.create(modelName = m['ormClassName'], localMaxTime = ut)
        db.close()
    ...
    def syncData(self):
        ...
        mgr = DbTableManager()
        datas = []
        for m in self.LocalSyncModel.select():
            mi = mgr.getModelInfo(m.modelName)
            model = mi['ormClass']
            qr = model.select().where(model.updateTime > m.localMaxTime).dicts()
            for rr in qr:
                datas.append(self.LocalSyncDataModel(modelName = m.modelName, value = json.dumps(rr)))
        self.LocalSyncDataModel.bulk_create(datas, 50)
        db.close()
    ...
    def update(self):
        ...
        mgr = DbTableManager()
        mdatas = {}
        for m in self.LocalSyncDataModel.select():
