    ...
    def loadHotTcAnchorsGroup(self):
        ...
        for one in tcs:
            for it in one:
                k = KEY(it)
                if k not in tks:
                    tks[k] = {'code': it['code'], 'name': it['name'], 'up': it['up'], 'num': 0, 'todayUp': 0, 'todayDown': 0}
                tks[k]['num'] += 1
        ...
        for m in todayHotTc:
            key = f"{m['code']}_{m['up']}"
            mht[key] = mht.get(key, 0) + 1
        for m in rs:
            m['todayUp'] = mht.get(f"{m['code']}_True", 0)
            m['todayDown'] = mht.get(f"{m['code']}_False", 0)
        return rs
